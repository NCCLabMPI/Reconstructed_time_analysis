...
pacman::p_load('dplyr', 'ggdist', 'ggeffects', 'ggpubr', 'lme4', 'emmeans', 'rstatix', 'car', 'rsq', 'sjPlot', 'pwr')
...
# plotting Figure 8a and b for experiment 2
correlation plots RTvis ~ iRTvis and RTaud ~ iRTaud
- as preregistred
...
# plotting Figure 8c for experiment 2
RTaud, RTvis, iRTaud and iRTvis as a function of onset SOA, grouped by duration and split by task-relevance
- as preregistred

```{R plotting_fig8c_exp2}
...
# collapse duration 
RTvis_table_fig8c = RTvis_table_fig8c %>% mutate(duration = ifelse(SOA_lock == 'onset',0,duration))

# take mean and standard deviation of RTvis for each condition
RTvis_table_fig8c = RTvis_table_fig8c %>%  group_by(onset_SOA, SOA_lock, task_relevance, duration) %>%
                                      summarise(mean_RT_vis = mean(RT_mean_vis, na.rm = TRUE),
                                                         sd_RT_vis = sd(RT_mean_vis, na.rm = TRUE),
                                                         sem_RT_vis = sd_RT_vis / sqrt(n2))


# remove NaNs from auditory responses
RTvis_table_fig8c = RTvis_table_fig8c[!is.na(RTvis_table_fig8c$mean_RT_vis), ]

# Define order and relabel levels for the task_relevance variable
RTaud_table_fig8c$task_relevance <- factor(RTaud_table_fig8c$task_relevance,
                                   levels = c('target', 'non-target', 'irrelevant'),
                                   labels = c('Target', 'Relevant non-target', 'Irrelevant non-target'))
RTvis_table_fig8c$task_relevance <- factor(RTvis_table_fig8c$task_relevance,
                                   levels = c('target'),
                                   labels = c('Target'))

# start plotting
fig8c = ggplot(data = RTaud_table_fig8c, aes(x = onset_SOA * 1000, y = mean_RT, group = interaction(as.character(duration), SOA_lock), linetype = SOA_lock, color = 'RT aud')) +
  # RTvis
  geom_line(data = RTvis_table_fig8c, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'RT vis', linetype = SOA_lock),linewidth = 1) +
  geom_errorbar(data = RTvis_table_fig8c, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, ymin = (mean_RT_vis - sem_RT_vis) * 1000, ymax = (mean_RT_vis + sem_RT_vis) * 1000, color = 'RT vis'), width = 30, linewidth = 1) +
  geom_point(data = RTvis_table_fig8c, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'RT vis', linetype = SOA_lock), size = 1.5) +
  # RT aud
  geom_line(linewidth = 1) +
  geom_point(size = 1.5) +
  geom_errorbar(aes(ymin = (mean_RT - sem_RT), ymax = (mean_RT + sem_RT)), width = 30, linewidth = 1) +
...
# specify the file name for the SVG and png export
svg_filename = "exp1_fig8c.svg"
png_filename = "exp1_fig8c.png"
