...
```{r Experiment 2 Kolmogorov smirnoff test to compare ECDF}
...
for (lck in soa_lock_conditions) {
  for (tsk in task_relevance_conditions) {
    if (lck == "onset") {
      # Filter data for onset conditions
      data_filtered <- all_event_table2 %>% 
        filter(SOA_lock == lck & task_relevance == tsk)
...
# Save these results to a table:
write.csv(results, file = file.path(save_dir, "Experiment2-kolmogorov-smirnoff.csv"), row.names = TRUE)
results
...
```{r Experiment 2 Target trials}

# Filter target  trials:
target_trials2 = target_event_table2 %>% filter(task_relevance == 'target')

# Convert SOA to ordinal factor:
target_trials2 = target_trials2 %>% mutate(f_SOA = factor(SOA, ordered = TRUE, levels = c("0", "0.232", "0.466")))
...
print('=================================')
print('ONSET:')
target_onset_trials2 = target_trials2 %>% filter(SOA_lock == 'onset')

# Fit onset model without the effect of duration due to fitting issues:
target_onset_mdl2 = glmer(RT_aud ~ f_SOA + (1*f_SOA | sub_id), 
                          family = Gamma(link="identity"), data = target_onset_trials2)
summary(target_onset_mdl2)
# get chisquare and p values 
anova_results <- Anova(target_onset_mdl2)
write.csv(anova_results, file = file.path(save_dir, "Experiment2-target-onset_mdl_anova.csv"), row.names = TRUE)
anova_results

# ======================================================================
# Offset trials:
target_offset_trials2 = target_trials2 %>% filter(SOA_lock == 'offset')

# =======================================
# Short trials:
print('=================================')
print('OFFSET SHORT:')
target_offset_short_trials2 = target_offset_trials2 %>% filter(duration == 0.5)
# Modelling:
target_offset_short_model2 = glmer(RT_aud ~ f_SOA+
                                     (1*f_SOA | sub_id), 
                                   family = Gamma(link="identity"), data = target_offset_short_trials2)
summary(target_offset_short_model2)
# Anova
anova_results <- Anova(target_offset_short_model2)
write.csv(anova_results, file = file.path(save_dir, "Experiment2-target-offset_short_mdl_anova.csv"), row.names = TRUE)
anova_results
...
print('=================================')
print('OFFSET INT:')
target_offset_int_trials2 = target_offset_trials2 %>% filter(duration == 1.0)
# Modelling:
target_offset_int_model2 = glmer(RT_aud ~ f_SOA+
                                   (1*f_SOA | sub_id), 
                                 family = Gamma(link="identity"), data = target_offset_int_trials2)
summary(target_offset_int_model2)
# Anova
anova_results <- Anova(target_offset_int_model2)
write.csv(anova_results, file = file.path(save_dir, "Experiment2-target-offset_int_mdl_anova.csv"), row.names = TRUE)
anova_results
...
print('=================================')
print('OFFSET LONG:')
target_offset_long_trials2 = target_offset_trials2 %>% filter(duration == 1.5)
# Modelling:
target_offset_long_model2 = glmer(RT_aud ~ f_SOA+
                                    (1*f_SOA | sub_id), 
                                  family = Gamma(link="identity"), data = target_offset_long_trials2)
summary(target_offset_long_model2)
# Anova
anova_results <- Anova(target_offset_long_model2)
write.csv(anova_results, file = file.path(save_dir, "Experiment2-target-offset_long_mdl_anova.csv"), row.names = TRUE)
anova_results
```
...
# joined SD
RT_aud_effect_size_table[4, ] = c(sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'target') %>% pull(RT_aud),target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' & task_relevance == 'target' & duration == 1.5) %>% pull(RT_aud) / 1000)),
                                  sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(RT_aud) / 1000, target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(RT_aud))),
                                  sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(RT_aud) / 1000, target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(RT_aud) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 0.5) %>% pull(RT_aud) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 0.5) %>% pull(RT_aud) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1) %>% pull(RT_aud) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1) %>% pull(RT_aud) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1.5) %>% pull(RT_aud) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1.5) %>% pull(RT_aud) / 1000)))
# ==================================================
# cohens d 
...
# ==================================================
# mean of SOA = 0
IT_aud_effect_size_table[1, ] = c(mean(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'target') %>% pull(iRT_aud) / 1000, na.rm = TRUE),
                                  mean(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_aud) / 1000, na.rm = TRUE), 
                                  mean(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_aud) / 1000, na.rm = TRUE), 
                                  mean(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 0.5) %>% pull(iRT_aud) / 1000, na.rm = TRUE), 
                                  mean(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1) %>% pull(iRT_aud) / 1000, na.rm = TRUE), 
                                  mean(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1.5) %>% pull(iRT_aud) / 1000, na.rm = TRUE))

# ==================================================
# mean of SOA = 466
IT_aud_effect_size_table[2, ] = c(mean(target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' & task_relevance == 'target' & duration == 1.5) %>% pull(iRT_aud) / 1000, na.rm = TRUE),
                                  mean(target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_aud) / 1000, na.rm = TRUE),
                                  mean(target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_aud) / 1000, na.rm = TRUE),
                                  mean(all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 0.5) %>% pull(iRT_aud) / 1000, na.rm = TRUE),
                                  mean(all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1) %>% pull(iRT_aud) / 1000, na.rm = TRUE),
                                  mean(all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1.5) %>% pull(iRT_aud) / 1000, na.rm = TRUE))
...
# joined SD
IT_aud_effect_size_table[4, ] = c(sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'target') %>% pull(iRT_aud),target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' & task_relevance == 'target' & duration == 1.5) %>% pull(iRT_aud) / 1000)),
                                  sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_aud) / 1000, target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_aud) / 1000)),
                                  sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_aud) / 1000, target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_aud) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 0.5) %>% pull(iRT_aud) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 0.5) %>% pull(iRT_aud) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1) %>% pull(iRT_aud) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1) %>% pull(iRT_aud) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1.5) %>% pull(iRT_aud) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1.5) %>% pull(iRT_aud) / 1000)))
...
# ==================================================
# mean of SOA = 0
IT_vis_effect_size_table[1, ] = c(mean(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'target') %>% pull(iRT_vis) / 1000, na.rm = TRUE),
                                  mean(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_vis) / 1000, na.rm = TRUE), 
                                  mean(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_vis) / 1000, na.rm = TRUE), 
                                  mean(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 0.5) %>% pull(iRT_vis) / 1000, na.rm = TRUE), 
                                  mean(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1) %>% pull(iRT_vis) / 1000, na.rm = TRUE), 
                                  mean(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1.5) %>% pull(iRT_vis) / 1000, na.rm = TRUE))

# ==================================================
# mean of SOA = 466
IT_vis_effect_size_table[2, ] = c(mean(target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' & task_relevance == 'target' & duration == 1.5) %>% pull(iRT_vis) / 1000, na.rm = TRUE),
                                  mean(target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_vis) / 1000, na.rm = TRUE),
                                  mean(target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_vis) / 1000, na.rm = TRUE),
                                  mean(all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 0.5) %>% pull(iRT_vis) / 1000, na.rm = TRUE),
                                  mean(all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1) %>% pull(iRT_vis) / 1000, na.rm = TRUE),
                                  mean(all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1.5) %>% pull(iRT_vis) / 1000, na.rm = TRUE))
...
# joined SD
IT_vis_effect_size_table[4, ] = c(sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'target') %>% pull(iRT_vis) / 1000,target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' & task_relevance == 'target' & duration == 1.5) %>% pull(iRT_vis)  / 1000)),
                                  sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_vis)  / 1000, target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'non-target') %>% pull(iRT_vis)  / 1000)),
                                  sd(c(target_event_table2 %>% filter(SOA == 0 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_vis) / 1000, target_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'onset' & task_relevance == 'irrelevant') %>% pull(iRT_vis) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 0.5) %>% pull(iRT_vis) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 0.5) %>% pull(iRT_vis) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1) %>% pull(iRT_vis) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1) %>% pull(iRT_vis) / 1000)),
                                  sd(c(all_event_table2 %>% filter(SOA == 0 & SOA_lock == 'offset' & duration == 1.5) %>% pull(iRT_vis) / 1000, all_event_table2 %>% filter(SOA == 0.466 & SOA_lock == 'offset' &  duration == 1.5) %>% pull(iRT_vis) / 1000)))
...
In the beginning of the second experiment, participants performed a calibration task in which they had to estimate the duration ofa beep and received feedback
...
for (subNum in subNums2){
  for (ses in sessions2){
    ...
    sub_folder = paste0('sub-', Lab_ID, subNum)
    ses_folder = paste0('ses-', ses)
    file_name = paste0(sub_folder, '_', ses_folder, '_run-1_task-', task,'_events.csv')
    
    # make file directory 
    file = file.path('P:/2023-0357-ReconTime/03_data/raw_data', sub_folder, ses_folder, file_name)
    
    # load file
    sub_tbl <- read.csv(file)
    # Add the subjects' ID
    sub_tbl$sub_id <- sub_folder
    # Add to the rest:
    calibration_tbl <- bind_rows(calibration_tbl, sub_tbl)
  }
}

# Convert to continuous variables:
calibration_tbl$iT <- as.numeric(calibration_tbl$iT) / 1000
calibration_tbl$cali_ms <- as.numeric(calibration_tbl$cali_ms) / 1000

# Linear mixed model:
# Calculate Pearson correlation coefficient
correlation <- cor.test(calibration_tbl$iT, calibration_tbl$cali_ms, method = "pearson")
...
# Plot the results:
ggplot(calibration_tbl, aes(x = iT, y = cali_ms, color = sub_id)) +
  geom_point() +
  labs(title = "Plot of iT vs cali_ms", x = "iT", y = "cali_ms") +
  theme_minimal()

# residual plots for visual inspection
plot(fitted(calibration_mdl),residuals(calibration_mdl))
qqnorm(residuals(calibration_mdl))
# get R square
rsq.glmm(calibration_mdl)
