...
```{r loading}
# details of subjects  
subNums = c(105, 106, 109, 110, 113, 114, 115, 118, 122) # 101
sessions = c(2,3)
Lab_ID = 'SX'
task = 'introspection'
count = 0
...
p13 <- ggplot(data = SOA_SOA_lock_data4, aes(x = onset_SOA * 1000, y = mean_RT, group = interaction(as.character(duration), SOA_lock), linetype = SOA_lock, color = 'RT aud')) +
  ...
  geom_line(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'RT vis', linetype = SOA_lock)) +
  geom_errorbar(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, ymin = (mean_RT_vis - sem_RT_vis) * 1000, ymax = (mean_RT_vis + sem_RT_vis) * 1000, color = 'RT vis'), width = 20) +
  geom_point(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'RT vis', linetype = SOA_lock)) +
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin = (mean_RT - sem_RT), ymax = (mean_RT + sem_RT)), width = 20) +
  geom_line(aes(y = mean_iT_aud, linetype = SOA_lock, color = 'IT aud')) +
  geom_point(aes(y = mean_iT_aud, color = 'IT aud')) +
  geom_errorbar(aes(ymin = (mean_iT_aud - sem_iT_aud), ymax = (mean_iT_aud + sem_iT_aud), color = 'IT aud'), width = 20) +
  geom_line(aes(y = mean_iT_vis, linetype = SOA_lock, color = 'IT vis')) +
  geom_point(aes(y = mean_iT_vis, color = 'IT vis')) +
  facet_grid(. ~ task_relevance) +
...
cor_aud = ggscatter(target_event_table , x = "z_RT_aud", y = "iRT_aud",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coef.coord = c(-2, 1000), cor.method = "pearson")+
  labs(title = "Estimation performance auditory task ", tag = "B")+
  xlab("z-scored objective reaction time [ms]")+
  ylab('Subjective  reaction time [ms]')+
  xlim(c(-2,5))+
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
...
cor_vis = ggscatter(target_event_table , x = "z_RT_vis", y = "iRT_vis",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coef.coord = c(-2, 1000), cor.method = "pearson")+
  labs(title = "Estimation performance visual task (T1) ", tag = "A")+
  xlab("z-scored objective areaction time [ms]")+
  ylab('Subjective reaction time [ms]')+
  xlim(c(-2,5))+
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
...
(cor_vis + cor_aud + plot_layout(ncol = 2, heights = c(1,3)))  /  (p13 + plot_layout(ncol = 1, heights = 10))
...
model_T1 = glmer(RT_aud ~ f_SOA*c_is_onset*c_task_relevant*z_iRT_vis +
                   (1*f_SOA*c_is_onset*c_task_relevant*z_iRT_vis| sub_id)+
                   (1*f_SOA*c_is_onset*c_task_relevant*z_iRT_vis | f_duration),
                 family=Gamma(link="identity"), data=all_event_table)
...
cali_sub = ggplot(all_cali_table, aes(x = cali_ms, y = iT))+
  geom_point() +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.border = element_rect(linetype = "solid"),
        panel.background = element_blank())+
  stat_cor(color = 'black', label.y = 1200, method = "pearson", show.legend = FALSE)
cali_sub
