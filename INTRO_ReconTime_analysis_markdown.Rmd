...
```{r loading}
# details of subjects  
subNums = c(101, 105, 106, 109, 110, 113, 114, 118, 122)
sessions = c(2,3)
Lab_ID = 'SX'
task = 'introspection'
count = 0
...
ggscatter(raw_event_table , x = "iRT_aud", y = "RT_aud",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson")


# mean of subject data
RT_iRT_data = raw_event_table %>% group_by(sub_id) %>%
...
aov_model <- aov(RT_aud ~ f_SOA*f_duration, data=offset_event_table)
summary(aov_model)
turky_pwc = TukeyHSD(aov_model, conf.level=.95)
turky_pwc
...
offset_f_SOA_data = offset_event_table %>%  group_by(f_SOA, sub_id) %>%
                                      summarise(mean_RT = mean(RT_aud, na.rm = TRUE))


pwc_data <- data.frame(mean_RT = offset_f_SOA_data$mean_RT, SOA = factor(offset_f_SOA_data$f_SOA, ordered = FALSE))   


# Pairwise comparisons
direct_pwc <- pwc_data %>% 
  pairwise_t_test(
    mean_RT ~ SOA, paired = TRUE,
...
raw_event_table = raw_event_table %>% mutate(f_SOA = factor(SOA, ordered = TRUE, levels = c("0", "0.232", "0.466")))


ggscatter(raw_event_table, x = "iRT_vis", y = "RT_aud",
...
# needs to be split by SOA lock
ggscatter(raw_event_table, x = "iRT_vis", y = "RT_aud",
   color = "f_SOA", palette = "jco",
   add = "reg.line", conf.int = TRUE, cor.coef = TRUE, cor.method = "pearson") + stat_cor(aes(color = f_SOA), label.y = c(2.6,2.4,2.2))


ggscatter(raw_event_table, x = "iRT_vis", y = "RT_aud",
   color = "task_relevance", palette = "jco",
   add = "reg.line", conf.int = TRUE, cor.coef = TRUE, cor.method = "pearson") + stat_cor(aes(color = task_relevance), label.y = c(2.6,2.4,2.2))
```
