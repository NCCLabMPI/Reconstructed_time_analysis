...
for (sub_num in subNums) {
  perf = perf %>% add_row(sub_num = as.character(sub_num),
                          hits = sum(target_event_table$trial_response_vis =='hit' & target_event_table$sub_num == sub_num),
                          misses = sum(target_event_table$trial_response_vis =='miss' & target_event_table$sub_num == sub_num),
                          crs = sum(target_event_table$trial_response_vis =='cr' & target_event_table$sub_num == sub_num),
                          fas = sum(target_event_table$trial_response_vis =='fa' & target_event_table$sub_num == sub_num),
                          hit_rate = hits/(hits+misses)*100,
                          fa_rate = fas/(fas+crs)*100,
                          aud_acc = mean(target_event_table$trial_accuracy_aud[target_event_table$sub_num == sub_num], na.rm = TRUE)*100,
...
cor_sin =ggscatter(target_event_table , x = "z_RT_aud", y = "iRT_aud",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coef.coord = c(-2, 1000), cor.method = "pearson")+
  labs(title = "Estimation performance single trials ", tag = "A")+
  xlab("Mean z-scored objective auditory reaction time [ms]")+
  ylab('Mean subjective auditory reaction time [ms]')+
  xlim(c(-2,1000))+
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
...
cor_sub = ggscatter(RT_iRT_data, x = "mean_RT", y = "mean_iRT", 
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.coef.coord = c(190, 0.7), cor.method = "pearson")+
