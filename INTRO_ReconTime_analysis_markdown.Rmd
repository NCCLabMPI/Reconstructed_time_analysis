...
p1 = ggplot(data=SOA_SOA_lock_data, aes(x=SOA*1000, y=mean_RT, group=SOA_lock, linetype = SOA_lock, color='objective RT')) +
  ...
  scale_linetype_manual(values=c("solid", "dashed"), name = 'SOA time-locked to', breaks=c('onset', 'offset'))+
    theme_bw() +
  scale_color_manual(values=c('black', 'deeppink', 'darkgreen'), name = 'Measure', breaks=c('objective RT', 'IT aud', 'IT vis'))+
  theme(axis.line = element_line(colour = "black"),
    panel.grid.major = element_blank(),
    panel.grid.minor = element_blank(),
    panel.border = element_rect(linetype = "solid"),
...
ggbarplot(task_mean_iRT_vis, x = "task_relevance", y = "mean_iRT", 
  ...
  scale_fill_manual(values = c("red","blue","green"), name = 'Task Relevance', labels=c('irrelevant', 'task-relevant non-target', 'target'))
...
c2a = ggscatter(target_event_table[target_event_table$SOA_lock == 'onset', ], x = "iRT_vis", y = "RT_aud",
  ...
  stat_cor(aes(color = f_SOA), label.y = c(2.6,2.4,2.2), method = "pearson", show.legend = FALSE) +
  ylim(c(0,3))+
  ggtitle("onset")+
  ylab("Objective auditory reaction time [ms]")+
  xlab('Introspective visual decision time [ms]')

c2b = ggscatter(target_event_table[target_event_table$SOA_lock == 'offset', ], x = "iRT_vis", y = "RT_aud",
   color = "f_SOA", palette = "jco", add = "reg.line", conf.int = TRUE, cor.coef = TRUE, cor.method = "pearson") + 
  stat_cor(aes(color = f_SOA), label.y = c(2.6,2.4,2.2),method = "pearson", show.legend = FALSE) + 
  ylim(c(0,3))+
  ggtitle("offset")+
  ylab("Objective auditory reaction time [ms]")+
...
c2_legend = get_legend(c2b)

ggarrange(c2a, c2b, ncol=2, common.legend = TRUE, legend="bottom", legend.grob = c2_legend)
...
c3a = ggscatter(target_event_table[target_event_table$SOA_lock == 'onset', ], x = "iRT_vis", y = "RT_aud",
  ...
  stat_cor(aes(color = task_relevance), label.y = c(2.6,2.4,2.2), show.legend = FALSE) +
  ylim(c(0,3)) +
  ggtitle("onset")+
  ylab("Objective auditory reaction time [ms]")+
  xlab('Introspective visual decision time [ms]')
...
c3_legend = get_legend(c3b)

ggarrange(c3a, c3b, ncol=2, common.legend = TRUE, legend="bottom", legend.grob = c3_legend)
