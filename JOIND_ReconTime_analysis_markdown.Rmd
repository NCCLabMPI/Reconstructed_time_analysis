...
```{r correlation2}
p_values = 1:length(subNums[subNums != 122])
...
p_values[count] = correlation$p.value
significance[count] = correlation$p.value < 0.05


# shuffle order and ran correlation to generate null distribution
null_correlation = cor.test(sample(RT_prp),sample(RT_intro), method = "pearson")
null_estimates[count] = null_correlation$estimate
null_p_values[count] = null_correlation$p.value
null_significance[count] = null_correlation$p.value < 0.05
}

# Combine data into a data frame
estimates_data <- data.frame(
  Group = rep(c("estimates", "null_estimates"), each = length(estimates)),
  Value = c(estimates, null_estimates)
)

# Create a boxplot
boxplot <- ggplot(estimates_data, aes(x = Group, y = Value, fill = Group)) +
  geom_boxplot() +
  geom_point() +
  labs(title = "Correlation estimates", y = "Value") +
  theme_minimal()
boxplot

# compare them with a t-test
ttest_result <- t.test(estimates, null_estimates, paired = TRUE)
