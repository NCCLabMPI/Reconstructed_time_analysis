...
sum_rp_data = rp_data %>%  group_by(SOA, SOA_lock) %>%
                                      summarise(mean_RT = mean(mean_RT, na.rm = TRUE),
                                                         sd_RT = sd(mean_RT, na.rm = TRUE))

sum_data <- summarySE(rp_data, measurevar = "mean_RT", groupvars=c("SOA_lock", "SOA"))
...
ggplot(rp_data, aes(x = SOA, y = mean_RT, fill = SOA_lock)) +
  ...
  geom_line(data = rp_data %>% filter(SOA_lock =="offset"), aes(x=SOAj+.02, y = mean_RT, group=sub_id), color = 'pink',linetype = 1, size=0.1)+ 
  ...
  # error bars
  geom_errorbar(data = sum_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock, ymin = mean_RT_mean-sem, ymax = mean_RT_mean+sem), width = .005)+ 
  # mean line
  geom_line(data = sum_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock),linetype = 2, size=0.7)+
  # mean points
  geom_point(data = sum_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock), shape = 18, size=3)+
  # adjust colors 
...
# exploratory offset analayses - models
