...
# remove RT > 700 ms
# all_event_table = all_event_table[all_event_table$RT_aud < 0.7, ]
...
boxplot(box_plot_data,las=2, par(mar=c(15,4,2,2)))
...
# general linear models with  a single fixed effect

model_SOA = glm(RT_aud ~ SOA, data=event_table)
summary(model_SOA)

model_duration = glm(RT_aud ~ duration, data=event_table)
summary(model_duration)

model_SOA_lock = glm(RT_aud ~ SOA_lock, data=event_table)
summary(model_SOA_lock)

model_task_relevance = glm(RT_aud ~ task_relevance, data=event_table)
summary(model_task_relevance)

# general linear models with fixed effects 

main_model = glm(RT_aud ~ SOA*SOA_lock*duration*task_relevance, family = Gamma(), data=event_table)
summary(main_model)

# general linear mixed effect model

main_mixed_model = glmer(RT_aud ~ SOA*SOA_lock*duration*task_relevance + (1+SOA+SOA_lock+duration+task_relevance|sub_id), family = Gamma(), data=all_event_table)
...
# effect size 
effect_size_t = t * sqrt(1/n)

eta = (f * df_num ) / (f * df_num  + df_denom )
effect_size_f = sqrt((df_denom/n)*(eta/(1-eta)))
...
from_F <- pwr.f2.test(u = u, v = NULL, f2 = effect_size_f, sig.level = alpha, power = power)
