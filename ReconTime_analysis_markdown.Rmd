...
from_F <- pwr.f2.test(u = u, v = NULL, f2 = effect_size, sig.level = alpha, power = power)
...
main_mixed_model = lmer(RT_aud ~ SOA*is_onset + (1|sub_num), data=all_event_table)
summary(main_mixed_model)

# Run simulation simr

# Extent data set
model_extended <- extend(main_mixed_model, along="sub_num", n = 20)

# POWER ANALYSIS WITH SIMR
powerC <- powerCurve(fit = model_extended, test = fixed("SOA","is_onset"), along = "sub_num", breaks = c(4,5,6))
   
print(powerC)
     
# RUN SIMULATION mixedpower
power_FLP <- mixedpower(model = main_mixed_model, data = all_event_table,
                         fixed_effects = c("SOA", "is_onset"),
                         simvar = "sub_num", steps = c(5,10,15,20),
