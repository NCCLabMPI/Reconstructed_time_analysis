...
sum_all_data <- summarySE(all_data, measurevar = "mean_RT", groupvars=c("SOA_lock", "SOA"))

light_colors = c(rep("pink",5),rep("lightblue",5))

#Rainclouds for repeated measures, additional plotting options 

ggplot(all_data, aes(x = SOA, y = mean_RT, fill = SOA_lock)) +
  ...
  geom_point(aes(x = as.numeric(SOA)-.02, y = mean_RT, colour = SOA_lock),position = position_jitter(width = .005), size = .25, shape = 20)+
  geom_boxplot(aes(x = SOA, y = mean_RT, fill = interaction(SOA_lock,SOA)),outlier.shape = NA, alpha = .5, width = .02, colour = "black")+geom_line(data = sum_all_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock), linetype = 3)+
  geom_point(data = sum_all_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock), shape = 18) +
  geom_errorbar(data = sum_all_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock, ymin = mean_RT_mean-sd, ymax = mean_RT_mean+sd), width = .005)+
 scale_fill_manual(values=light_colors)+scale_color_manual(values = c("onset" = "blue", "offset" = "red"))
...
reduced_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant + (1*c_SOA*is_onset*c_duration*is_task_relevant|sub_id), family=Gamma(link="log"), data=all_event_table)
