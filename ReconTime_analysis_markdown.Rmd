...
if (!('emmeans' %in% installed.packages()))
...
if (!('rsq' %in% installed.packages()))
{install.packages('rsq', dependencies = TRUE)}
library(rsq)
...
if (!('glmm.hp' %in% installed.packages()))
{install.packages('glmm.hp', dependencies = TRUE)}
library(glmm.hp)
...
full_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant +
                     (1*c_SOA*is_onset*c_duration*is_task_relevant|sub_id),
...
reduced_model = glmer(RT_aud ~ c_SOA*is_onset*is_task_relevant +
                        (1*c_SOA*is_onset*is_task_relevant|sub_id),
                      family=Gamma(link="identity"), data=all_event_table)
...
rsq.glmm(reduced_model)
```


# split by SOA-lock analayses - models
...
offset_model = glmer(RT_aud ~ c_SOA*c_duration*is_task_relevant +
                       (1*c_SOA*c_duration*is_task_relevant | sub_id) +                     
                     (1*c_SOA*c_duration*c_task_relevant | c_pitch) +
                     (1*c_SOA*c_duration*c_task_relevant | category),
...
# post-hoc pairwise comparison with factorial SOA and duration

f_offset_event_table = offset_event_table %>% mutate(f_SOA = factor(SOA, ordered = TRUE, levels = c("0", "0.116", "0.232", "0.466")))
f_offset_event_table = f_offset_event_table %>% mutate(f_duration = factor(duration, ordered = TRUE, levels = c("0.5", "1", "1.5")))

f_offset_model = glmer(RT_aud ~ f_SOA*f_duration*c_task_relevant +
                       (1*f_SOA*f_duration*c_task_relevant | sub_id) +                     
                     (1*f_SOA*f_duration*c_task_relevant | c_pitch) +
                     (1*f_SOA*f_duration*c_task_relevant | category),
                     family=Gamma(link="identity"), data=f_offset_event_table)

Anova(f_offset_model)
rsq.glmm(f_offset_model)

em1 <- emmeans(f_offset_model, "f_SOA", by = "f_duration")
contrast(em1, "pairwise", adjust = "bonferroni")

em2 <- emmeans(f_offset_model, "f_SOA")
...
onset_model = glmer(RT_aud ~ c_SOA*c_duration*is_task_relevant +
                       (1*c_SOA*c_duration*is_task_relevant | sub_id) +                     
                     (1*c_SOA*c_duration*c_task_relevant | c_pitch) +
                     (1*c_SOA*c_duration*c_task_relevant | category),
...
# pitch  
pitch_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant*c_pitch + (1*c_SOA*is_onset*c_duration*is_task_relevant*c_pitch|sub_id), family=Gamma(link="identity"), data=all_event_table)
summary(pitch_model)

# category 
cate_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant*cate_num + (1*c_SOA*is_onset*c_duration*is_task_relevant*cate_num|sub_id), family=Gamma(link="identity"), data=all_event_table)
summary(cate_model)

# orientation 
ori_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant*ori_num + (1*c_SOA*is_onset*c_duration*is_task_relevant*ori_num|sub_id), family=Gamma(link="identity"), data=all_event_table)
summary(ori_model)
```

# exploratory more random effects 

```{r}

# with category and pitch as random effects 

mega_model = glmer(RT_aud ~ c_SOA*c_is_onset*c_duration*c_task_relevant +
                     (1*c_SOA*c_is_onset*c_duration*c_task_relevant | sub_id) +
                     (1*c_SOA*c_is_onset*c_duration*c_task_relevant | c_pitch) +
                     (1*c_SOA*c_is_onset*c_duration*c_task_relevant | category), 
                   family=Gamma(link="identity"), data=all_event_table)
summary(mega_model)
Anova(mega_model)
rsq.glmm(mega_model)
...
mega_model2 = glmer(RT_aud ~ c_SOA*c_is_onset*c_task_relevant +
                     (1*c_SOA*c_is_onset*c_task_relevant | sub_id) +
                     (1*c_SOA*c_is_onset*c_task_relevant | c_duration) +
                     (1*c_SOA*c_is_onset*c_task_relevant | c_pitch) +
                     (1*c_SOA*c_is_onset*c_task_relevant | category), 
                   family=Gamma(link="identity"), data=all_event_table)
summary(mega_model2)
Anova(mega_model2)
rsq.glmm(mega_model2)


# all fixed effects => converging issues 

#mega_model3 = glmer(RT_aud ~ c_SOA*c_is_onset*c_task_relevant*c_duration*c_pitch*category +
#                     (1*c_SOA*c_is_onset*c_task_relevant*c_duration*c_pitch*category | sub_id), 
#                   family=Gamma(link="identity"), data=all_event_table)

#summary(mega_model3)
#Anova(mega_model3)
#rsq.glmm(mega_model3)
```
...
#brm_SOA_SOA_lock_model = brm(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant + (1*c_SOA*is_onset*c_duration*is_task_relevant|sub_id), all_event_table, family=Gamma(link="identity"),save_pars = save_pars(all = TRUE))
...
reduced_event_table = reduced_event_table %>% mutate(c_SOA = SOA - mean(SOA))
reduced_event_table = reduced_event_table %>% mutate(c_duration = duration - mean(duration))
...
reduced_offset_model = glmer(RT_aud ~ c_SOA*is_task_relevant +
                               (1*c_SOA*is_task_relevant|sub_id) +
                               (1*c_SOA*is_task_relevant|c_duration),
                             family=Gamma(link="identity"), data=reduced_offset_event_table)

summary(reduced_offset_model)
anov = Anova(reduced_offset_model)

p[rep] = anov['c_SOA','Pr(>Chisq)']
