...
ggplot(all_data, aes(x = SOA, y = mean_RT, group = SOA)) + 
  ggdist::stat_halfeye(
    adjust = .9, 
    width = .1, 
    .width = 0, 
...
ggplot(rp_data, aes(x = SOA, y = mean_RT, fill = SOA_lock)) +
  ...
  geom_point(aes(x = as.numeric(SOAj)+.02, y = mean_RT, colour = SOA_lock), size = 1, shape = 20) +
  ...
  # mean points
  geom_point(data = sum_data, aes(x = as.numeric(SOA)+.02, y = mean_RT_mean, group = SOA_lock, colour = SOA_lock), shape = 18, size=1.5)+
  # error bars
...
full_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant + (1*c_SOA*is_onset*c_duration*is_task_relevant|sub_num), family=Gamma(link="log"), data=all_event_table)
summary(full_model)
...
reduced_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant + (1*c_SOA*is_onset*c_duration*is_task_relevant|sub_num), family=Gamma(link="log"), data=all_event_table)
...
# pitch  
pitch_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant*c_pitch + (1*c_SOA*is_onset*c_duration*is_task_relevant*c_pitch|sub_num), family=Gamma(link="log"), data=all_event_table)
summary(pitch_model)

# category 
cate_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant*cate_num + (1*c_SOA*is_onset*c_duration*is_task_relevant*cate_num|sub_num), family=Gamma(link="log"), data=all_event_table)
summary(cate_model)

# orientation 
ori_model = glmer(RT_aud ~ c_SOA*is_onset*c_duration*is_task_relevant*ori_num + (1*c_SOA*is_onset*c_duration*is_task_relevant*ori_num|sub_num), family=Gamma(link="log"), data=all_event_table)
summary(ori_model)
