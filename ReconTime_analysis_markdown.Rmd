...
ggplot(rp_data, aes(x = SOA, y = mean_RT, fill = SOA_lock)) +
  ...
  geom_point(aes(x = as.numeric(SOAj)+.02, y = mean_RT, colour = SOA_lock), size = 2, shape = 20) +
  # box plots
  ...
  # mean line
  geom_line(data = sum_data, aes(x = as.numeric(SOA)+.02, y = RT_mean, group = SOA_lock, colour = SOA_lock),linetype = 2, size=0.7)+
  # mean points
  geom_point(data = sum_data, aes(x = as.numeric(SOA)+.02, y = RT_mean, group = SOA_lock, colour = SOA_lock), shape = 18, size=3)+
  # adjust colors 
...
write.csv(summ['coefficients'],file='GZLM_CG_M1_result.csv')
```
...
onset_model = glmer(RT_aud ~ c_SOA*c_duration*is_task_relevant + (1*c_SOA*c_duration*is_task_relevant|sub_num), family=Gamma(link="identity"), data=offset_event_table)
summary(onset_model)
Anova(onset_model)
