...
def blink_latency(parameters_file, subjects, session="1", task="prp", analysis_name="blink_latency",
    ...
    latencies_lmm.to_csv(Path(save_dir, "blink_peak_latencies.csv"))
    ...
    for lock in param["lock"]:
        ...
        if lock == "onset":  # For the offset locked trials, we need to plot separately per stimuli durations:
            ...
            fig = plot_pupil_latency(evks, times, latencies, ev.colors["soa_{}_locked".format(lock)],
                                     ...
                                     figsize=[8.3, 11.7 / 3], smooth_ms=50)
            fig.savefig(Path(save_dir, "blink_latency_{}.svg".format(lock)), transparent=True, dpi=300)
            fig.savefig(Path(save_dir, "blink_latency_{}.png".format(lock)), transparent=True, dpi=300)
            plt.close()
            ...
            for task in param["task_relevance"]:
                ...
                fig = plot_pupil_latency(evks, times, latencies, ev.colors["soa_{}_locked".format(lock)],
                                         boxplot_ylim=boxplot_ylim, pupil_size_ylim=pupil_size_ylim,
                                         figsize=[8.3, 11.7 / 3], smooth_ms=50)
                fig.savefig(Path(save_dir, "blink_latency_{}-{}.svg".format(lock, task)), transparent=True, dpi=300)
                fig.savefig(Path(save_dir, "blink_latency_{}-{}.png".format(lock, task)), transparent=True, dpi=300)
                plt.close()
        ...
        for dur_i, duration in enumerate(param["duration"]):
            ...
            fig = plot_pupil_latency(evks, times, latencies, ev.colors["soa_{}_locked".format(lock)],
                                     ...
                                     figsize=[8.3, 11.7/4], smooth_ms=50)
            fig.savefig(Path(save_dir, "blink_latency_{}-{}.svg".format(lock, duration)),
                        transparent=True, dpi=300)
            fig.savefig(Path(save_dir, "blink_latency_{}-{}.png".format(lock, duration)),
                        transparent=True, dpi=300)
            plt.close()
        ...
        for task in param["task_relevance"]:
            for dur_i, duration in enumerate(param["duration"]):
                ...
                fig = plot_pupil_latency(evks, times, latencies, ev.colors["soa_{}_locked".format(lock)],
                                         ...
                                         plot_legend=plot_legend,
                                         figsize=[8.3, 11.7/4], smooth_ms=50)
                fig.savefig(Path(save_dir, "blink_latency_{}-{}-{}.svg".format(lock, task, duration)),
                            transparent=True, dpi=300)
                fig.savefig(Path(save_dir, "blink_latency_{}-{}-{}.png".format(lock, task, duration)),
                            transparent=True, dpi=300)
                plt.close()
...
if __name__ == "__main__":
    ...
    parameters = (
        ...
        r"\05-ET_blinks_latency_parameters.json")
    # ==================================================================================
    # Introspection analysis:
    task = "introspection"
    blink_latency(parameters, ev.subjects_lists_et[task], task=task, session=["2", "3"],
                  analysis_name="blink_latency", reject_bad_trials=True)

    # ==================================================================================
    # PRP analysis:
    task = "prp"
    blink_latency(parameters, ev.subjects_lists_et[task], task=task, session="1",
                  analysis_name="blink_latency", reject_bad_trials=True)
