...
# difference
sum(raw_event_table$SOA_lock == 'onset') - sum(target_event_table$SOA_lock == 'onset')
sum(raw_event_table$SOA_lock == 'offset') - sum(target_event_table$SOA_lock == 'offset')

# relative trial removal
(sum(raw_event_table$SOA_lock == 'onset') - sum(target_event_table$SOA_lock == 'onset'))/sum(raw_event_table$SOA_lock == 'onset')
(sum(raw_event_table$SOA_lock == 'offset') - sum(target_event_table$SOA_lock == 'offset'))/sum(raw_event_table$SOA_lock == 'offset')
...
for (sub_num in subNums) {
  perf = perf %>% add_row(sub_num = as.character(sub_num),
                          ...
                          RT_aud = mean(raw_event_table$RT_aud[raw_event_table$sub_num == sub_num], na.rm = TRUE)*1000,
                          RT_vis = mean(raw_event_table$RT_vis[raw_event_table$sub_num == sub_num], na.rm = TRUE)*1000)
}
...
p24 = ggplot(data = SOA_SOA_lock_duration_data4, aes(x = onset_SOA * 1000, y = mean_RT * 1000, group = interaction(as.character(duration), SOA_lock), color = as.character(duration), linetype = SOA_lock)) +
  ...
  geom_point(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'darkgrey', group = interaction(as.character(duration), SOA_lock)), size = 1) +
  geom_line() +
  geom_errorbar(aes(ymin = (mean_RT - sem_RT) * 1000, ymax = (mean_RT + sem_RT) * 1000), width = 20) +
  geom_point(size = 1) +
  facet_grid(. ~ task_relevance) +
...
RT_dis = ggplot(all_event_table, aes(x = z_RT_aud, color = as.factor(SOA))) +
  stat_ecdf(geom = "step") +
  xlab("z-scored auditory reaction time [ms]") +
  ylab("Cumulative Probability") +
  labs(title = "Auditory reaction time distribution", tag = "B") +
  xlim(c(-2,3))+
  scale_color_manual(values = c('red', 'blue', 'green', "orange"), name = 'SOA [ms]', labels=c('0', '116', '232', '466'))+
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
# open grid.arrange library
library(gridExtra)

# combine p24 and RT_dis to one plot
exp1 = grid.arrange(p24, RT_dis, nrow = 2)
