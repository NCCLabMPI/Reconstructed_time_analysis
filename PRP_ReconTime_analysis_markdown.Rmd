...
# --------------------- #  
SOA_SOA_lock_duration_subj_data4 = all_event_table %>% group_by(onset_SOA,SOA_lock, duration, sub_id, task_relevance) %>%
                                      summarise(mean_RT = mean(RT_aud[{q<-rank(RT_aud)/length(RT_aud);q>=0.9}], na.rm = TRUE),
                                                mean_RT_reg = mean(RT_aud, na.rm = TRUE),
                                                         sd_RT = sd(RT_aud, na.rm = TRUE),
                                                         sem_RT = sd_RT / sqrt(n))

# remove NaNs from auditory responses
SOA_SOA_lock_duration_subj_data4 = SOA_SOA_lock_duration_subj_data4[!is.na(SOA_SOA_lock_duration_subj_data4$mean_RT), ]


p23 = ggplot(data=SOA_SOA_lock_duration_subj_data4, aes(x=onset_SOA*1000, y=mean_RT*1000, group=interaction(as.character(duration), SOA_lock), color=as.character(duration), linetype = SOA_lock)) +
  geom_line()+
  geom_errorbar(aes(ymin = (mean_RT-sem_RT)*1000, ymax = (mean_RT+sem_RT)*1000), width = 20)+
  geom_point(size = 1)+
  labs(title = "Objective reaction time by task-relevance", tag = "A") +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.border = element_rect(linetype = "solid"),
        panel.background = element_blank()) +   
  facet_grid(sub_id ~ task_relevance)
p23
...
rp_data = all_event_table %>%  group_by(SOA, SOA_lock, sub_id) %>%
                                      summarise(SOA = SOA*1000,
                                                mean_RT = mean(z_RT_aud, na.rm = TRUE)*1000,
                                                         sd_RT = sd(z_RT_aud, na.rm = TRUE)*1000)
...
rcp2 = ggplot(all_event_table, aes(x = SOA*1000, y = z_RT_aud*1000, fill = SOA_lock)) +
  # individual data points
  geom_jitter(aes(x = SOA*1000, y = z_RT_aud*1000, colour = SOA_lock), size = 0.1, width = 12) +
  # distribution plot
  ...
  geom_point(data = sum_data, aes(x = as.numeric(SOA)+18, y = RT_mean, group = SOA_lock, colour = SOA_lock), shape = 18, size=1)+
    # boxplot
  geom_boxplot(aes(x = SOA*1000-20,y = z_RT_aud*1000, fill = interaction(SOA_lock,SOA)),outlier.shape = NA, alpha = .5, width = 10, colour = "black")+
  # adjust colors
