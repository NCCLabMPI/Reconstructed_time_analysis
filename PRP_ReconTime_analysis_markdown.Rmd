...
SOA_SOA_lock_duration_subj_data3 = target_event_table %>% group_by(onset_SOA,SOA_lock, duration, sub_id, task_relevance) %>%
                                      summarise(RT_mean = mean(RT_aud, na.rm = TRUE))
...
SOA_SOA_lock_duration_data4$task_relevance = factor(SOA_SOA_lock_duration_data4$task_relevance,
                                   ...
                                   labels = c('Target', 'Task-relevant non-target', 'Task-irrelevant non-target'))
...
p24 = ggplot(data = SOA_SOA_lock_duration_data4, aes(x = onset_SOA * 1000, y = mean_RT * 1000, group = interaction(as.character(duration), SOA_lock), color = as.character(duration), linetype = SOA_lock)) +
  geom_line(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'darkgrey', group = interaction(as.character(duration), SOA_lock), linetype = SOA_lock)) +
  geom_errorbar(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, ymin = (mean_RT_vis - sem_RT_vis) * 1000, ymax = (mean_RT_vis + sem_RT_vis) * 1000, group = interaction(as.character(duration), SOA_lock), color = 'darkgrey', width = 20)) +
   geom_point(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'RT vis', group = interaction(as.character(duration), SOA_lock), linetype = SOA_lock)) +
  geom_line() +
  geom_errorbar(aes(ymin = (mean_RT - sem_RT) * 1000, ymax = (mean_RT + sem_RT) * 1000), width = 20) +
  geom_point(size = 1) +
  labs(title = "Objective reaction time by task-relevance", tag = "A") +
...
RT_dis = ggplot(all_event_table, aes(x = z_RT_aud, color = as.factor(SOA))) +
  ...
  theme(axis.line = element_line(colour = "black"),
        ...
        panel.border = element_rect(linetype = "solid"),
        panel.background = element_blank()) +   
  facet_grid(SOA_lock ~ task_relevance)
RT_dis
