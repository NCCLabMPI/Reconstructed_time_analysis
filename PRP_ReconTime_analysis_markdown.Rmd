...
RT_dis = ggplot(target_event_table, aes(x = z_RT_aud, color = as.factor(SOA))) +
  stat_ecdf(geom = "step") +
...
layout <- rbind(c(3), c(2))
grid.arrange(p24, RT_dis, ncol = 3, nrow = 2, widths = c(3, 2))
...
ggplot(data=acc_data, aes(x=SOA*1000, y=mean_acc*100, group=interaction(SOA_lock, task_relevance)))+
