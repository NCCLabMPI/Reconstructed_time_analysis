...
# z-score of RT_vis and RTaud and division in high an low
...
vis_data4 = target_event_table %>% group_by(onset_SOA,SOA_lock, sub_id, task_relevance) %>%
                                      summarise(RT_mean_vis = mean(RT_vis, na.rm = TRUE))

vis_data4 = vis_data4 %>%  group_by(onset_SOA, SOA_lock, task_relevance) %>%
...
# define order
SOA_SOA_lock_duration_data4$task_relevance = factor(SOA_SOA_lock_duration_data4$task_relevance , levels=c('target','non-target','irrelevant', labels = c('Target', 'Task-relevant non-target', 'Task-irrelevant non-target')))
vis_data4$task_relevance = factor(vis_data4$task_relevance , levels=c('target','non-target','irrelevant', labels = c('Target', 'Task-relevant non-target', 'Task-irrelevant non-target')))
...
p24 = ggplot(data = SOA_SOA_lock_duration_data4, aes(x = onset_SOA * 1000, y = mean_RT * 1000, group = interaction(as.character(duration), SOA_lock), color = as.character(duration), linetype = SOA_lock)) +
  geom_line(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'darkgrey', group = SOA_lock, linetype = SOA_lock)) +
  geom_errorbar(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, ymin = (mean_RT_vis - sem_RT_vis) * 1000, ymax = (mean_RT_vis + sem_RT_vis) * 1000, group = SOA_lock, color = 'darkgrey', width = 20)) +
  geom_line() +
  geom_errorbar(aes(ymin = (mean_RT - sem_RT) * 1000, ymax = (mean_RT + sem_RT) * 1000), width = 20) +
  geom_point(size = 1) +
  labs(title = "Objective reaction time by task-relevance", tag = "A") +
