...
wd = 'C:/Users/JLU-SU/OneDrive - Justus-Liebig-Universität Gießen/Dokumente/GitHub/Reconstructed_time_analysis'
...
{event_table$X <- NULL}
if ('TargetScreenOnset' %in% colnames(event_table))
{event_table$TargetScreenOnset <- NULL}
...
# summary(all_event_table$resp_window[all_event_table$SOA == 0.466 & all_event_table$SOA_lock == 'offset' & all_event_table$duration == 1.5], na.rm = TRUE)
# summary(all_event_table$resp_window[all_event_table$SOA == 0 & all_event_table$SOA_lock == 'onset' & all_event_table$duration == 0.5], na.rm = TRUE)
# 
# sum(target_event_table$SOA == 0.466 & target_event_table$SOA_lock == 'offset' & target_event_table$duration == 1.5 & target_event_table$task_relevance == 'target')
# sum(target_event_table$SOA == 0 & target_event_table$SOA_lock == 'onset' & target_event_table$duration == 0.5 & target_event_table$task_relevance == 'irrelevant')
# 
# task_relevance_lvl2 = c('non-target', 'irrelevant', 'target')
# 
# 
# for (iii in task_relevance_lvl2) {
#   for (i in SOA_lvl) {
#     for (ii in SOA_lock_lvl) {
#       for (iv in duration_lvl) {
#         
#         cell_trials = sum(target_event_table$SOA == i & target_event_table$SOA_lock == ii & target_event_table$task_relevance == iii & target_event_table$duration == iv, na.rm = TRUE)
#         
#         
#         cell_name = paste(i,ii,iii,iv,sep= '-')
#         print(paste0(cell_trials,' trials', ' for cell: ', cell_name))
#         
#       }}}}
...
# sum(target_event_table$SOA == 0)
# sum(target_event_table$SOA == 0.466)
# sum(target_event_table$duration == 0.5)
# sum(target_event_table$duration == 1.5)
# sum(target_event_table$task_relevance == 'irrelevant')
# sum(target_event_table$task_relevance == 'non-target')
# sum(target_event_table$task_relevance == 'target')
...
perf = perf %>% add_row(sub_num = 'sd',
                        hits = sd(perf$hits[1:21]),
                        misses = sd(perf$misses[1:21]),
                        crs = sd(perf$crs[1:21]),
                        fas = sd(perf$fas[1:21]),
                        hit_rate = sd(perf$hit_rate[1:21]),
                        fa_rate = sd(perf$fa_rate[1:21]),
                        aud_acc = sd(perf$aud_acc[1:12]),
                        vis_acc = sd(perf$vis_acc[1:12]),
                        RT_aud = sd(perf$RT_aud[1:21]),
                        RT_vis = sd(perf$RT_vis[1:21]),
                        RT_aud_raw = sd(perf$RT_aud_raw[1:21]),
                        RT_vis_raw = sd(perf$RT_vis_raw[1:21]))
...
# by duration, extended time - by task relevance

SOA_SOA_lock_duration_subj_data3 = raw_event_table %>% group_by(onset_SOA,SOA_lock, duration, sub_id, task_relevance) %>%
                                      summarise(RT_mean = mean(RT_aud, na.rm = TRUE))

SOA_SOA_lock_duration_data3 = SOA_SOA_lock_duration_subj_data3 %>%  group_by(onset_SOA, SOA_lock,duration, task_relevance) %>%
                                      summarise(mean_RT = mean(RT_mean, na.rm = TRUE),
                                                         sd_RT = sd(RT_mean, na.rm = TRUE),
                                                         sem_RT = sd_RT / sqrt(n))

# remove NaNs from auditory responses
SOA_SOA_lock_duration_data3 = SOA_SOA_lock_duration_data3[!is.na(SOA_SOA_lock_duration_data3$mean_RT), ]

# define order
SOA_SOA_lock_duration_data3$task_relevance = factor(SOA_SOA_lock_duration_data3$task_relevance , levels=c('target','non-target','irrelevant'))
...
# including RT vis 
SOA_SOA_lock_duration_subj_data4 = target_event_table %>% group_by(onset_SOA,SOA_lock, duration, sub_id, task_relevance) %>%
                                      summarise(RT_mean = mean(RT_aud, na.rm = TRUE))
...
p24 = ggplot(data = SOA_SOA_lock_duration_data4, aes(x = onset_SOA * 1000, y = mean_RT * 1000, group = interaction(as.character(duration), SOA_lock), color = as.character(duration), linetype = SOA_lock)) +
  geom_line(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'darkgrey', group = interaction(as.character(duration), SOA_lock)), linewidth = 1) +
  geom_errorbar(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, ymin = (mean_RT_vis - sem_RT_vis) * 1000, ymax = (mean_RT_vis + sem_RT_vis) * 1000, group = interaction(as.character(duration), SOA_lock), color = 'darkgrey', width = 30), linewidth = 1) +
  geom_point(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'darkgrey', group = interaction(as.character(duration), SOA_lock)), size = 1.5) +
  geom_line(linewidth = 1) +
  geom_errorbar(aes(ymin = (mean_RT - sem_RT) * 1000, ymax = (mean_RT + sem_RT) * 1000), width = 30, linewidth = 1) +
  geom_point(size = 1.5) +
  facet_grid(. ~ task_relevance) +
  labs(title = "Objective reaction time by task-relevance", tag = "A") +
  ylab("Reaction time [ms]") +
...
ggplot(rp_data, aes(x = SOA, y = mean_RT, fill = SOA_lock)) +
  ...
  # adjust colors 
  scale_fill_manual(values=fill_colors, name = NULL, breaks = NULL) + scale_color_manual(values = line_colors,name = 'SOA time-locked to', breaks=c('onset', 'offset'))+
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
rp_data = all_event_table %>%  group_by(SOA, SOA_lock, sub_id) %>%
                                      summarise(SOA = SOA*1000,
                                                mean_RT = mean(RT_aud, na.rm = TRUE)*1000,
                                                         sd_RT = sd(RT_aud, na.rm = TRUE)*1000)
...
rp_data$SOAj = jitter(rp_data$SOA, amount = .5)


# define colors
fill_colors = c(rep("pink",8),rep("lightblue",8))
line_colors = c("onset" = "blue", "offset" = "red")

#Rainclouds for repeated measures, additional plotting options

rcp2 = ggplot(all_event_table, aes(x = SOA*1000, y = RT_aud*1000, fill = SOA_lock)) +
  # individual data points
  geom_jitter(aes(x = SOA*1000, y = RT_aud*1000, colour = SOA_lock), size = 0.1, width = 12) +
  # distribution plot
  geom_flat_violin(aes(fill = interaction(SOA_lock,SOA*1000)),position = position_nudge(x = 18, y = 0), adjust = 1.5, trim = FALSE, alpha = .5, colour = NA)+

  # error bars
  geom_errorbar(data = sum_data, aes(x = as.numeric(SOA)+18, y = RT_mean, group = SOA_lock, colour = SOA_lock, ymin = RT_mean-RT_sem, ymax = RT_mean+RT_sem), width = 5)+
  # mean line
  geom_line(data = sum_data, aes(x = as.numeric(SOA)+18, y = RT_mean, group = SOA_lock, colour = SOA_lock),linetype = 2, size=0.5)+
  # mean points
  geom_point(data = sum_data, aes(x = as.numeric(SOA)+18, y = RT_mean, group = SOA_lock, colour = SOA_lock), shape = 18, size=1)+
    # boxplot
  geom_boxplot(aes(x = SOA*1000-20,y = RT_aud*1000, fill = interaction(SOA_lock,SOA)),outlier.shape = NA, alpha = .5, width = 10, colour = "black")+
  # adjust colors
  ...
  ylab("Reaction time [ms]")+
  xlab('SOA [ms]')+
  labs(title = "Reaction time distribution", tag = "B") +
  theme_bw() +
  theme(axis.line = element_line(colour = "black"),
        panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
## general linear mixed effect model 1 (single trial level)
...
model1_dur = glmer(RT_aud ~ f_SOA*c_is_onset*c_task_relevant*f_duration +
                     (1*f_SOA*c_is_onset*c_task_relevant*f_duration | sub_id),
               family=Gamma(link="identity"), data=all_event_table)
...
# subject level model (mean of each cell for each subjects - no signle trials)
...
# with duration
onset_model_dur = glmer(RT_aud ~ f_SOA*c_task_relevant*f_duration +
                       (1*f_SOA*c_task_relevant*f_duration | sub_id), 
                     family = Gamma(link="identity"), data = onset_event_table)

summary(onset_model_dur)
Anova(onset_model_dur)
rsq.glmm(onset_model_dur)
...
em3 <- emmeans(offset_model, "f_SOA", by = c("f_duration", "c_task_relevant"))
contrast(em3, "pairwise", adjust = "bonferroni")

em4 <- emmeans(offset_model, c("f_SOA", "f_duration"))
contrast(em4, "pairwise", adjust = "bonferroni")

# MAYBE FIND ONE THAT DISPLAYS P VALUES WITH MORE VALUES AFTER THE PERIOD


# without duration
offset_model_no_dur = glmer(RT_aud ~ f_SOA*c_task_relevant +
                       (1*f_SOA*c_task_relevant | sub_id) + 
                      (1*f_SOA*c_task_relevant | f_duration), 
                     family = Gamma(link="identity"), data = offset_event_table)

summary(offset_model_no_dur)
Anova(offset_model_no_dur)
rsq.glmm(offset_model_no_dur)

emx <- emmeans(offset_model_no_dur, "f_SOA", by = "f_duration")
contrast(emx, "pairwise", adjust = "bonferroni")
...
p24 = ggplot(data = SOA_SOA_lock_duration_data4, aes(x = onset_SOA * 1000, y = mean_RT * 1000, group = interaction(as.character(duration), SOA_lock), color = as.character(duration), linetype = SOA_lock)) +
  geom_line(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, color = 'darkgrey', group = interaction(as.character(duration), SOA_lock))) +
  geom_errorbar(data = vis_data4, aes(x = onset_SOA * 1000, y = mean_RT_vis * 1000, ymin = (mean_RT_vis - sem_RT_vis) * 1000, ymax = (mean_RT_vis + sem_RT_vis) * 1000, group = interaction(as.character(duration), SOA_lock), color = 'darkgrey', width = 20)) +
  
  
# auditory accuracy plot
acc_data = raw_event_table %>%  group_by(SOA, SOA_lock, task_relevance, sub_id) %>%
                                      summarise(mean_acc = mean(trial_accuracy_aud, na.rm = TRUE),
                                                         sd_acc = mean(trial_accuracy_aud, na.rm = TRUE))

acc_data = acc_data %>%  group_by(SOA, SOA_lock, task_relevance) %>%
                                      summarise(mean_acc2 = mean(mean_acc, na.rm = TRUE),
                                                         sd_mean_acc = sd(mean_acc, na.rm = TRUE),
                                                         sem_mean_acc = sd_mean_acc / sqrt(n))


acc_plot = ggplot(data=acc_data, aes(x=SOA*1000, y=mean_acc2*100, colour = task_relevance, linetype = SOA_lock, group=interaction(SOA_lock, task_relevance)))+
  geom_line()+
  geom_point()+
  geom_errorbar(aes(ymin = (mean_acc2 - sem_mean_acc) * 100, ymax = (mean_acc2 + sem_mean_acc) * 100, width = 10))+
  ylim(c(85,100))+
  ylab("Accuracy in %")+
  xlab('SOA [ms]')+  
  scale_color_manual(values = c('red', 'blue', 'darkgreen'),
                                        name = 'Task-relevance',
                                        breaks = c('target', 'non-target', 'irrelevant'),
                                        labels = c('Target', 'Relevant non-target', 'Irrelevant non-target')) +
...
acc_data2 = raw_event_table %>% group_by(onset_SOA,SOA_lock, duration, sub_id, task_relevance) %>%
                                      summarise(acc_mean = mean(trial_accuracy_aud, na.rm = TRUE))

acc_data2 = acc_data2 %>%  group_by(onset_SOA, SOA_lock,duration, task_relevance) %>%
                                      summarise(mean_acc = mean(acc_mean, na.rm = TRUE),
                                                         sd_acc = sd(acc_mean, na.rm = TRUE),
                                                         sem_acc = sd_acc / sqrt(n))

# remove NaNs from auditory responses
acc_data2 = acc_data2[!is.na(acc_data2$mean_acc), ]

# define order
acc_data2$task_relevance = factor(acc_data2$task_relevance,
                                   levels = c('target', 'non-target', 'irrelevant'),
                                   labels = c('Target', 'Relevant non-target', 'Irrelevant non-target'))


acc_plot2 = ggplot(data = acc_data2, aes(x = onset_SOA * 1000, y = mean_acc, group = interaction(as.character(duration), SOA_lock), color = as.character(duration), linetype = SOA_lock)) +
  geom_line(linewidth = 1) +
...
## general linear mixed effect for accuracy 
...
sub_folder = paste0('sub-', Lab_ID, subNum)
ses_folder = 'ses-1'
file_name = paste0(sub_folder, '_ses-1_run-0_task-auditory_events.csv')
...
# load file
aud_pratice_table = read.csv(file)

# add subject number and session column 
aud_pratice_table = aud_pratice_table %>% mutate(sub_num = subNum)
aud_pratice_table = aud_pratice_table %>% mutate(sub_id = paste0(Lab_ID, subNum))
aud_pratice_table = aud_pratice_table %>% mutate(ses = 1)

# concatanate the tables
if (exists('all_aud_pratice_table')){
  all_aud_pratice_table = rbind(all_aud_pratice_table, aud_pratice_table)
} else {
  all_aud_pratice_table = aud_pratice_table
}
}

all_aud_pratice_table = all_aud_pratice_table %>% mutate(RT_aud = time_of_resp_aud - aud_stim_time)

# trial exclusion
# remove RT < 100 ms
all_aud_pratice_table = all_aud_pratice_table[all_aud_pratice_table$RT_aud > 0.1, ]

# remove NaNs from auditory responses
all_aud_pratice_table = all_aud_pratice_table[!is.na(all_aud_pratice_table$RT_aud), ]

mean_RTaud_practice = mean(all_aud_pratice_table$RT_aud, na.rm = TRUE)
sd_RTaud_practice = sd(all_aud_pratice_table$RT_aud, na.rm = TRUE)
