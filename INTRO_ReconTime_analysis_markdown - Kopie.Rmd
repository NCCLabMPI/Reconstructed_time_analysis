...
perf = perf %>% add_row(sub_num = 'sd',
                        hits = sd(perf$hits[1:9]),
                        misses = sd(perf$misses[1:9]),
                        crs = sd(perf$crs[1:9]),
                        fas = sd(perf$fas[1:9]),
                        hit_rate = sd(perf$hit_rate[1:9]),
                        fa_rate = sd(perf$fa_rate[1:9]),
                        aud_acc = sd(perf$aud_acc[1:9]),
                        vis_acc = sd(perf$vis_acc[1:9]),
                        RT_aud = sd(perf$RT_aud[1:9]),
                        RT_vis = sd(perf$RT_vis[1:9]),
                        RT_aud_raw = sd(perf$RT_aud_raw[1:9]),
                        RT_vis_raw = sd(perf$RT_vis_raw[1:9]))
```
