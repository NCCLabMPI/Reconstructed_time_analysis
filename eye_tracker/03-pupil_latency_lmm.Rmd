...
full_path = file.path(bids_root, 'derivatives', 'pupil_size', 'group_level', 'data', 'peak_latencies.csv')
...
```{r Full model:}
fullmodel <- lmer(formula = peak_latency ~ 1 + task_relevance * lock * soa + (1 + task_relevance * lock * soa | subject),
                  data = data)
summary(fullmodel)
anova(fullmodel)
```
...
onset_model <- lmer(formula = peak_latency ~ 1 + task_relevance * soa * duration + ( 1 + task_relevance * soa | subject),
                  data = onset_data)
summary(onset_model)
anova(onset_model)
