...
# Convert the task to a categorical variable:
data_exp1 = data_exp1 %>% mutate(f_task = factor(task, ordered = FALSE))

# Modelling:
model_exp1 <- lmer(formula = RT2 ~ 1 + pupil_latency * task + (pupil_latency | sub_id), 
                         data = data_exp1)
summary(model_exp1)
...
# Convert the task to a categorical variable:
data_exp2 = data_exp2 %>% mutate(f_task = factor(task, ordered = FALSE))

# Modelling:
model_exp2 <- lmer(formula = RT2 ~ 1 + pupil_latency * task + (pupil_latency | sub_id), 
                         data = data_exp2)
summary(model_exp2)
