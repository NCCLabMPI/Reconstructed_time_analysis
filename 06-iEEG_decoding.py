...
from helper_function.helper_general import extract_first_bout, create_super_subject, get_roi_channels, get_cmap_rgb_values
import pandas as pd
...
def decoding(parameters_file, subjects, data_root, session="1", task="dur", analysis_name="decoding",
    ...
    for roi in param["rois"]:
        ...
        save_dir = Path(ev.bids_root, "derivatives", analysis_name, task, roi_name)
        ...
        ax.set_title("Decoding over time in {} (# channels=)".format(roi_name, n_channels))
        ...
        if decoding_latencies["Relevant non-target"][0] is not None and decoding_latencies["Irrelevant"][0] is not None:
            ...
            offset_diff = decoding_latencies["Relevant non-target"][0] - decoding_latencies["Irrelevant"][0]
        else:
            onset_diff = None
            offset_diff = None
    ...
    onset_colors = get_cmap_rgb_values(latencies_table["onset tr-ti"].to_numpy(), cmap="Reds", center=0)
    # Plot the brain:
    Brain = mne.viz.get_brain_class()
    brain = Brain('fsaverage', hemi='lh', surf='inflated',
                  subjects_dir=ev.fs_directory, size=(800, 600))
    # Loop through each label:
    for lbl_i, lbl in enumerate(latencies_table["roi"].to_list()):
        brain.add_label(lbl + "-lh", color=onset_colors[lbl_i])
    for view in views:
        brain.show_view(view)
        brain.save_image('%s_%s.png' % ("onset", view))
    brain.close()

    # ===============
    # offset:
    # Get colors for each label:
    offset_colors = get_cmap_rgb_values(latencies_table["offset tr-ti"].to_numpy(), cmap="Blues", center=0)
    # Plot the brain:
    Brain = mne.viz.get_brain_class()
    brain = Brain('fsaverage', hemi='lh', surf='inflated',
                  subjects_dir=ev.fs_directory, size=(800, 600))
    # Loop through each label:
    for lbl_i, lbl in enumerate(latencies_table["roi"].to_list()):
        brain.add_label(lbl + "-lh", color=offset_colors[lbl_i])
    for view in views:
        brain.show_view(view)
        brain.save_image('%s_%s.png' % ("onset", view))
    brain.close()
